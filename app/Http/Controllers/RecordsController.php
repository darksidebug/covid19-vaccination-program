...
class RecordsController extends Controller
{
    ...
    public function getrecords(Request $request){
        ...
        {
            return response()->json(['data' => $this->usersRecords($request->input('table-municipality'),$request->input('date_filter'))]);
        }else
        {
            return response()->json(['data'=> $this->vaccinatedRecords($request->input('table-municipality'),$request->input('date_filter'))]);
        }
    }

    public function vaccinatedRecords($municipality,$datefilter){
        $users = DB::table('people')
                    ...
                    ->where('vaccinateds.created_at', 'LIKE', '%'. $datefilter .'%')
                    ->get();

        return $users;
    }

    public function usersRecords($municipality,$datefilter){
